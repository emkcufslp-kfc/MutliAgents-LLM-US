...
class HedgeFundGraph:
    ...
    def _node_fundamental_analyst(self, state: AgentState) -> Dict[str, Any]:
        ...
        system_msg = SystemMessage(content="You are a strict Hedge Fund Fundamental Analyst. Review the data and provide a concise 2-sentence thesis.")
    ...
    def _node_bull_researcher(self, state: AgentState) -> Dict[str, Any]:
        ...
        system_msg = SystemMessage(content="You are the Bull Researcher. Argue fiercely for why this stock is a BUY based on the analyst reports.")
    ...
    def _node_bear_researcher(self, state: AgentState) -> Dict[str, Any]:
        ...
        system_msg = SystemMessage(content="You are the Bear Researcher. Argue fiercely for why this stock is a SELL/AVOID based on the analyst reports.")
        user_msg = HumanMessage(content=f"Ticker: {state['ticker']}\nReports: {state.get('analyst_reports', {})}")
    ...
    def _node_portfolio_manager(self, state: AgentState) -> Dict[str, Any]:
        ...
        system_msg = SystemMessage(content="You are the Portfolio Manager. Review the Analyst Reports and Debate Transcript. Reply strictly with a JSON-like structure: FINAL_DECISION: [BUY/HOLD/SELL], CONFIDENCE: [0.0-1.0], REASON: [Your thesis].")
        user_msg = HumanMessage(content=f"Ticker: {state['ticker']}\nReports: {state.get('analyst_reports', {})}\nDebate: {state.get('debate_transcript', {})}")
        
        content = self._invoke_llm(self.tier2_llm, [system_msg, user_msg], "FINAL_DECISION: BUY, CONFIDENCE: 0.85, REASON: Technical breakout supported by strong FCF. (Simulated Fallback)")
        
        # A simple parser for the sake of the demo
        decision = "BUY" if "BUY" in content.upper() else "SELL" if "SELL" in content.upper() else "HOLD"
        
        return {
            "final_decision": decision,
            "confidence": 0.80, # Ideally extracted from response
